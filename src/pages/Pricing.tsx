...
import { Building2, User, Check, Star, Crown, Infinity, ArrowLeft, Moon, Sun } from 'lucide-react';
...
const Pricing = () => {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/20 to-gray-100 dark:from-slate-900 dark:via-slate-900/95 dark:to-slate-800">
      {/* Header */}
            ...
            <TabsContent value="hospitals" className="space-y-12">
                  ...
                  <div className="max-w-md mx-auto">
                    <div className="flex items-center gap-4 mb-4">
                      <Select value={doctorCount.toString()} onValueChange={value => setDoctorCount(parseInt(value))}>
                        ...
                        <SelectContent>
                          {[1, 2, 3, 4, 5, 10, 15, 20, 25, 50, 100].map(num => <SelectItem key={num} value={num.toString()}>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
                  {hospitalCreditBundles.map((bundle, index) => <Card key={index} className="relative bg-white/80 dark:bg-slate-800/80 backdrop-blur-lg border-gray-200 dark:border-gray-700">
                      {bundle.popular && <Badge className="absolute -top-2 left-1/2 transform -translate-x-1/2 bg-orange-500">
                          <Star className="h-3 w-3 mr-1" />
                          Popular
                        </Badge>}
                      {bundle.bestValue && <Badge className="absolute -top-2 left-1/2 transform -translate-x-1/2 bg-green-500">
            ...
            <TabsContent value="individual" className="space-y-12">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
                  {individualPlans.map((plan, index) => <Card key={index} className={`relative bg-white/80 dark:bg-slate-800/80 backdrop-blur-lg border-gray-200 dark:border-gray-700 ${plan.name === 'Elite' ? 'ring-2 ring-medical-primary scale-105' : ''}`}>
                      {plan.popular && <Badge className="absolute -top-2 left-1/2 transform -translate-x-1/2 bg-orange-500">
                          <Star className="h-3 w-3 mr-1" />
                          Popular
                        </Badge>}
                      <CardHeader className="text-center">
                      ...
                      <CardContent>
                        <div className="text-center mb-6">
                          {plan.unlimited ? <div className="flex items-center justify-center gap-2 text-lg font-semibold">
                              <Infinity className="h-5 w-5" />
                              Unlimited usage
                            </div> : <div className="text-lg font-semibold">{plan.credits} credits/month</div>}
                        </div>
